...
struct expty transExp(S_table venv, S_table tenv, A_exp a){
	switch(a->kind){
		...
		case A_breakExp: return expTy(NULL, Ty_Void());
		case A_opExp: {
		...
		case A_callExp: {
			...
			for(; parms && expected; parms = parms->tail, expected = expected->tail){
				struct expty exp =transExp(venv, tenv, parms->head);
			...
			return expTy(NULL, base_ty(x->u.fun.result));
		}
		case A_recordExp: {
			...
			for(; params && expected; params = params->tail, expected = expected->tail){
				...
				struct expty exp = transExp(venv, tenv, params->head->exp);
				if(base_ty(expected->head->ty) != base_ty(exp.ty)) EM_error(a->pos, "type not match");
			}
		...
		case A_assignExp: {
			...
			//EM_error(1, "%d, %d", lvalue.ty->kind, exp.ty->kind);
		...
		case A_ifExp: {
			...
			if(a->u.iff.elsee){
				elseexp = transExp(venv, tenv, a->u.iff.elsee);
				if(base_ty(then.ty) != base_ty(elseexp.ty)){
	...
	switch(v->kind){
		case A_simpleVar: {
			...
			if (!x || x->kind != E_varEntry){
				...
				return expTy(NULL, base_ty(x->u.var.ty));
			}
		}
		case A_fieldVar: {
			struct expty value = transVar(venv, tenv, v->u.field.var);
			...
			return expTy(NULL, base_ty(fileds->head->ty));
		}
		case A_subscriptVar: {
			struct expty value = transVar(venv, tenv, v->u.subscript.var);
			...
			return expTy(NULL, base_ty(value.ty->u.array));
		}
	}
}
...
void transDec(S_table venv, S_table tenv, A_dec d){
	switch(d->kind){
		...
		case A_typeDec: {
			...
			while(types){
				if (S_look(tenv, types->head->name) != NULL) EM_error(d->pos, "two types have the same name");else S_enter(tenv, types->head->name, Ty_Name(types->head->name, NULL));
				types = types->tail;
			}
		...
		case A_functionDec: {
			...
			while(funcs){
				...
				struct expty exp = transExp(venv, tenv, funcs->head->body);
