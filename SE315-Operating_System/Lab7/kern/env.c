...
int
env_alloc_base(struct Env **newenv_store, envid_t parent_id){
	int32_t generation;
	int r;
	struct Env *e;
...
__user_mapped_text int
env_alloc(struct Env **newenv_store, envid_t parent_id)
{   
    int r = env_alloc_base(newenv_store, parent_id);
	return r;
}
...
load_icode(struct Env *e, uint8_t *binary){   
    ...
    if(ELFHDR->e_magic != ELF_MAGIC) panic("The ELF not executable!");

    ph = (struct Proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);
    eph = ph + ELFHDR->e_phnum;

    lcr3(PADDR(e->env_pgdir));
    e->env_heap = (uintptr_t)ROUNDUP(UTEXT, PGSIZE);
    for(; ph < eph; ph++){
        if(ph->p_type == ELF_PROG_LOAD){
            ...
            if(ph->p_va + ph->p_memsz > e->env_heap){
                e->env_heap = (uintptr_t)ROUNDUP(ph->p_va + ph->p_memsz, PGSIZE);
            }
        }
    }
