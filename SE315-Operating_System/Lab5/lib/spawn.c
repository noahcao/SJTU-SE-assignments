...
copy_shared_pages(envid_t child)
{
    ...
    uintptr_t addr;
    int r;
    // search all ptes under current process and copy all pages
    // with PTE_SHARE set into the child processes
    for(addr = UTEXT; addr < USTACKTOP; addr += PGSIZE){
        if((uvpd[PDX(addr)] & PTE_P) && (uvpt[PGNUM(addr)] 
        & (PTE_P|PTE_U|PTE_SHARE)) == (PTE_P|PTE_U|PTE_SHARE)){
            if((r = sys_page_map((envid_t)0, (void *)addr, child, (void *)addr, uvpt[PGNUM(addr)] & PTE_SYSCALL)) < 0)
                panic("sys_page_map in copy_shared_pages: %e", r);
        }
    }
	return 0;
}
