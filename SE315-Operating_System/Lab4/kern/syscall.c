...
// Returns the csurrent environment's envid.
static envid_t
sys_getenvid(void)
{
...
sys_exofork(void)
{
	...
	new_env->env_heap = curenv->env_heap;
...
//	-E_BAD_ENV if environment envid doesn't currently exist,
//		or the caller doe  sn't have permission to change envid.
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (srcva != ROUNDUP(srcva, PGSIZE) || (uintptr_t)srcva >= UTOP || dstva != ROUNDUP(dstva, PGSIZE) || (uintptr_t)dstva >= UTOP)	
		return -E_INVAL;
	
	pte_t* pte;
	struct PageInfo *p = page_lookup(src_env->env_pgdir, srcva, &pte);
    bool cond1 = !(perm & PTE_U);
    bool cond2 = !(perm & PTE_P);
    bool cond3 = (perm & ~PTE_SYSCALL);
    bool cond4 = (perm & PTE_W);
    bool cond5 = !(*pte & PTE_W);

	if (!p || cond1 || cond2 || cond3)	return -E_INVAL;
	if(cond4 && cond5) return -E_INVAL;
	if(page_insert(dst_env->env_pgdir, p, dstva, perm)<0)
		return -E_NO_MEM;
	return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	if(page_lookup(env->env_pgdir,va,NULL)==NULL)
		return 0;
...
sys_sbrk(uint32_t inc){
	...
	for(int i=0;i<size;i+=PGSIZE){
		struct PageInfo *p = page_alloc(0);
		if (!p) panic("sys_sbrk : alloc page error\n");
