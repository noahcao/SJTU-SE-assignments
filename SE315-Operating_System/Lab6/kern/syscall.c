...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP){
		if (ROUNDUP(srcva, PGSIZE) == srcva && (perm & PTE_SYSCALL)  ){
...
sys_net_send(const void *buf, uint32_t len)
{
    ...
    int r;
    r = user_mem_check(curenv, buf, len, PTE_U | PTE_P);
    if(r < 0) return r;
...
int
sys_read_mac(uint8_t *macaddr){
    user_mem_assert(curenv, macaddr, 6, PTE_W);
    memcpy(macaddr, e1000_mac_address, 6);
    return 0;
}
