...
void fuseserver_symlink(fuse_req_t req, const char* linkname, fuse_ino_t parent, const char* filename){
    struct fuse_entry_param e;
    ...
    int status = yfs->symlink(parent, linkname, filename, inum);
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino){
    ...
    int status = yfs->readlink(ino, path);
    if(status != yfs_client::OK){
        fuse_reply_err(req, status);
    }
    else{
