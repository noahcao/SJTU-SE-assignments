...
getattr(yfs_client::inum inum, struct stat &st)
{
    ...
    if(yfs->isfile(inum)){
        ...
        st.st_nlink = 1;
        st.st_atime = (unsigned int)time(NULL);
        st.st_mtime = (unsigned int)time(NULL);
        st.st_ctime = (unsigned int)time(NULL);
        ...
        int to_set, struct fuse_file_info *fi)
{
    ...
    // f**king code FUSE_SET_ATTR_SIZE & to_set always 
...
fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name){
    ...
    if((ret = yfs->symlink( link, parent, name, inum)) == yfs_client::OK){
        e.ino = inum;
        getattr(inum, e.attr);
        fuse_reply_entry(req, &e);
    }
