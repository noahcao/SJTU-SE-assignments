...
int DataNode::init(const string &extent_dst, const string &namenode, const struct sockaddr_in *bindaddr) {
  ...
  NewThread(this,&DataNode::Heartbeat,id);
  return 0;
}

void DataNode::Heartbeat(DatanodeIDProto id){
  while (true){
    SendHeartbeat();
    sleep(3);
  }
...
bool DataNode::ReadBlock(blockid_t bid, uint64_t offset, uint64_t len, string &buf) {
  string full_buf;
  ec->read_block(bid, full_buf);
  buf = full_buf.substr(offset, len);
  return true;
}
...
bool DataNode::WriteBlock(blockid_t bid, uint64_t offset, uint64_t len, const string &buf) {
  ...
  string target_seg = content.replace(offset,len,buf);
  ec->write_block(bid, target_seg);
  return true;
}
