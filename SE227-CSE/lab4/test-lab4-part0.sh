...
check_network() {
  ...
  if [[ "x$myname" != "xapp" && "x$myname" != "xname" && "x$myname" != "xdata1" && "x$myname" != "xdata2" ]]; then
    echo "You didn't setup host name correctly1"
    exit 1
  fi
  ...
  if ! (grep -v "^#" /etc/hosts | grep app >/dev/null); then
    echo "You didn't setup /etc/hosts correctly2"
    exit 1
  fi
  if ! (grep -v "^#" /etc/hosts | grep name >/dev/null); then
    echo "You didn't setup /etc/hosts correctly3"
    exit 1
  fi
  if ! (grep -v "^#" /etc/hosts | grep data1 >/dev/null); then
    echo "You didn't setup /etc/hosts correctly4"
    exit 1
  fi
  if ! (grep -v "^#" /etc/hosts | grep data2 >/dev/null); then
    echo "You didn't setup /etc/hosts correctly5"
    exit 1
  fi
