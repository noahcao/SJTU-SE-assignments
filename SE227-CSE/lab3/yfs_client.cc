...
yfs_client::_create(inum parent, const char *name, mode_t mode, inum &ino_out)
{
    ...
    if(_lookup(parent, name, exist, tmp_num) == IOERR) return EXIST;
...
int yfs_client::symlink(inum parent, const char* linkname, const char* filename, inum& ino_out){
	lockme(parent);
	int ret = _symlink(parent, linkname, filename, ino_out);
	releaseme(parent);
	return ret;
}

int yfs_client::_symlink(inum parent, const char* linkname, const char* filename, inum& ino_out){
